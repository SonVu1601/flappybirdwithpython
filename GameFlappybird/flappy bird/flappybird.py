...
while running:
	...
	clock.tick(60)#nhay 60 lan 1 s
	screen.fill(WHITE)
	screen.blit(background_img,(0,0))
	#ép ống ảnh và vẽ ống top
	...
	#ép ống ảnh và vẽ ống bottom
	...
	#di chuyen sang trai
	tube1_x-=tube_valocity
	tube2_x-=tube_valocity
	tube3_x-=tube_valocity
	#ong moi
	...
	for event in pygame.event.get():
		if event.type==pygame.QUIT:
			running=False
		if event.type==pygame.KEYDOWN:#bam phim
			if event.key==pygame.K_SPACE:#bam phim space
				bird_drop_velocity=0
				...
				if pausing:
					...
					score=0
					pausing=False
	pygame.display.flip()#hienlenmanhinh
pygame.quit()